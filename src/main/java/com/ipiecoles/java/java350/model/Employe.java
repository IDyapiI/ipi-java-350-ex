...
public class Employe {
    ...
    /**
     * Retourne le nombre d'année d'ancienneté de l'employé par rapport à sa date
     * d'embauche (on ne prend pas en compte les mois et les jours. Il faut en
     * revanche que la d'embauche soit non nulle et l'année antérieure à l'année
     * courante sinon on renvoie une ancienneté de 0
     *
     * @return le nombre d'année d'ancienneté
     */
    public final Integer getNombreAnneeAnciennete() {
        return dateEmbauche != null && LocalDate.now().getYear() >= dateEmbauche.getYear()
                ? LocalDate.now().getYear() - dateEmbauche.getYear()
                : 0;
    }
    ...
    /**
     * Nombre de jours de RTT = Nombre de jours dans l'année – plafond maximal du
     * forfait jours de la convention collective – nombre de jours de repos
     * hebdomadaires – jours de congés payés – nombre de jours fériés tombant un
     * jour ouvré
     *
    ...
    public Double getNbRtt(LocalDate d) {
        int nbDayInAYear = d.isLeapYear() ? 366 : 365;
        int nbWeekEnd = 104;
        ...
        int nbOfPublicHolidays = (int) Entreprise.joursFeries(d).stream()
                .filter(localDate -> localDate.getDayOfWeek().getValue() <= DayOfWeek.FRIDAY.getValue()).count();
                
        int nbRtt = nbDayInAYear - Entreprise.NB_JOURS_MAX_FORFAIT - nbWeekEnd - Entreprise.NB_CONGES_BASE - nbOfPublicHolidays;
        if (tempsPartiel < 0)
            return 0.0;
